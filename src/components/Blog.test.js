...
describe('<Blog />', () => {
  ...
  test('display only title and author', () => {
    ...
    expect(container.querySelector('.title')).toHaveTextContent(`${blog.title} ${blog.author}`);
  ...
  test('display url and likes when view button is clicked', async() => {
    ...
    expect(container.querySelector('.details')).not.toHaveStyle('display: none');
    expect(container.querySelector('.details')).toHaveTextContent(`${blog.url}likes: ${blog.likes}like${blog.user.name}remove`);
  });
